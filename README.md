# Hi 👋 I'm P. Tamilselvan  

🎯 **Lead Technical Recruiter | Aspiring Data Analyst**  
📍 Coimbatore, India  

Aspiring Data Analyst with hands-on project experience and certified Data Analyst, skilled in SQL, Python, Power BI, Excel Pivot Tables, dashboards, and data visualization. Currently working as a Lead Technical Recruiter at Emergere Technologies LLC, managing end-to-end recruitment for multiple technology roles.
Brings strong analytical thinking, experience with data-driven metrics, trend analysis, and stakeholder reporting. Proven ability to interpret data, build insights, and present findings through clear visualizations. Actively seeking to transition into a Data Analyst role to apply technical and business skills in a data-focused environment.

---

## 🔹 About Me

- 💼 Lead Technical Recruiter at **Emergere Technologies** with 5+ years of exp
- 📊 Transitioning into **Data Analytics & Business Intelligence**
- 🔍 Strong experience in sourcing, stakeholder management, and analytics-driven decision making
- 🚀 Promoted to Lead role based on consistent performance
- 🎓 B.E. in **Aeronautical Engineering** (Anna University)

---

## 🛠️ Skills & Tools

**Data Analytics**
- MS Excel (Advanced)
- SQL
- Python (NumPy, Pandas)
- Power BI (DAX, KPIs, Dashboards)
- Data Cleaning & Visualization
- Descriptive & Inferential Statistics

**Recruitment & Business**
- Full Life Cycle Recruiting
- Talent Sourcing & Screening
- Stakeholder Management
- Client Handling
- Problem Solving & Business Analysis

---

## 📂 Projects

### 📊 Freshco Hypermarket – Excel Analytics
- Order-level, customer-level & delivery analysis  
- Built dashboards using Pivot Tables, Charts & logical metrics  

### ✈️ Airline Database – SQL
- Designed relational database for airline ticketing  
- Used Joins, Case Statements & Window Functions  
- Improved reporting and data retrieval efficiency  

### 🛍️ Shop Nest Store – Power BI
- Built interactive dashboards using DAX measures & KPIs  
- Enabled real-time filtering using slicers  

### 🐍 Employee Performance – Python
- Data cleaning and analysis using Pandas & NumPy  
- Identified top performers and skill gaps  

---

## 📜 Certifications

- 📘 **Data Analytics Course** – SkilloVilla (2025)
- 🎯 Tech Recruiter Certification – Hirist.com
- 🧠 Tech Recruitment Certified Professional – DevSkiller
- 📚 LinkedIn Learning: *Talent Sourcing* – Barbara Bruno

---

## 📫 Connect With Me

- 📧 Email: **thajuaero15@gmail.com**
- 💼 LinkedIn: [linkedin.com/in/tamilselvan-p](https://linkedin.com/in/tamilselvan-p)
- 🐙 GitHub: [github.com/Tamilselvan-2512](https://github.com/Tamilselvan-2512)

---

⭐ *If you like my work or find it useful, feel free to star my repositories!*
